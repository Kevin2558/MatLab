function [result] = SOR_Jacobi(A,b,x,w,precision,max_iteration)
%Metodo de Seidel
[m,n] = size(A);
process = false;
if m~=n
...
if (process)
    ...
    iter = 0;
    M = (1 - w).*eye(n)- w.*(D^-1)*(L+U);
    %Norma
    r = norm(M,Inf);
    fprintf('SOR - Jacobi - Norma Infinit0: %d\n',r);
    %Radio espectral
    radio_espectral = max(abs(eig(M)));
    fprintf('SOR -Jacobi - Radio Espectral: %d\n',radio_espectral);
    %iteraciones
    values = [];
    iteration = [];
    errors = [];
